  ...
  return (
    <Float speed={2} rotationIntensity={0.4} floatIntensity={0.8}>
      <mesh ref={meshRef} position={position}>
        ...
        <MeshDistortMaterial color={color} metalness={0.5} roughness={0.15} distort={0.3} speed={3} transparent opacity={0.6} />
      </mesh>
    </Float>
  );
}
...
export function SectionScene3D({ variant = 'default' }: { variant?: 'default' | 'products' | 'testimonials' }) {
  const colors = {
    default: { primary: '#a78bfa', secondary: '#67e8f9' },
    products: { primary: '#67e8f9', secondary: '#f0abfc' },
    testimonials: { primary: '#f0abfc', secondary: '#a78bfa' },
  };
  ...
  return (
    <div className="w-full h-[200px] relative">
      <Canvas camera={{ position: [0, 0, 5], fov: 40 }} dpr={[1, 1.5]}>
        <fog attach="fog" args={['#0f0d1a', 3, 15]} />
        <ambientLight intensity={0.4} />
        <pointLight position={[3, 3, 3]} intensity={0.6} color={c.primary} />
        <pointLight position={[-3, -2, 2]} intensity={0.3} color={c.secondary} />
